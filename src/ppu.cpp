...
void PPU::ProcessPixelPipeline() {
  ...
  // processed every other dot, to simulate each of the first 4 stages taking
  // two dots
  if (cycles_ % 2) {
    switch (fetchStep_) {
      case TILE:
      ...
      case PUSH:
        PushToFIFO();
        break;  // in the actual process, push happens every dot
    }
  }
...
void PPU::FetchTile() {
  if (GetLCDControl(kLCDControlBGWinEnable)) {
    ...
    if (WindowIsVisible() && fetcherX_ + 7 >= wx_ &&
      ...
      addr =
          GetWindowTileMapArea() + (fetcherX_ - wx_ + 7) / 8 + windowTileY * 32;
    } else {
    ...
    if (!GetLCDControl(
            kLCDControlBGWinTileDataAreaSelect)) {  // offset address based on
                                                    // addressing mode for tile
      tileNumber_ += 128;
    }
  }
  ...
  if (GetLCDControl(kLCDControlObjEnable) && spritesInLine_.size() > 0) {
    for (auto& spriteEntry : spritesInLine_) {
      ...
      // check if either the left or the right side of the sprite coincide with
      // the current x
...
void PPU::FetchTileDataLow() {
  ...
  oamDataLow_ = {};
  oamDataLow_.reserve(fetchedSprites_.size());

  uint8_t spriteHeight = GetSpriteHeight();
  for (int i = 0; i < fetchedSprites_.size(); i++) {
    uint8_t tileY = ((ly_ + 16) - fetchedSprites_[i].y) * 2;

    if (fetchedSprites_[i].yFlip) {
      tileY = (spriteHeight * 2) - 2 - tileY;
    }

    uint8_t tileIndex = fetchedSprites_[i].tileIndex;

    if (spriteHeight == 16) {
      tileIndex &= ~(0x01);
    }

    oamDataLow_[i] = memory_->Read(kLCDSpriteDataArea + tileIndex * 16 + tileY);
  }
...
void PPU::FetchTileDataHigh() {
  ...
  oamDataHigh_ = {};
  oamDataHigh_.reserve(fetchedSprites_.size());
  ...
  for (int i = 0; i < fetchedSprites_.size(); i++) {
    ...
    oamDataHigh_[i] =
        memory_->Read(kLCDSpriteDataArea + tileIndex * 16 + tileY + 1);
  }

  fetchStep_ = SLEEP;
}

void PPU::FetchSleep() { fetchStep_ = PUSH; }
...
void PPU::PushToFIFO() {
  ...
  for (int i = 7; i >= 0; i--) {
    ...
    // rather than mixing separately, just decide on the sprite pixel to show
    // here
    if (GetLCDControl(kLCDControlObjEnable)) {
      for (int i = 0; i < fetchedSprites_.size(); i++) {
        int spriteX = (fetchedSprites_[i].x - 8) + scx_ % 8;
        if (fifoPushedX_ > (spriteX + 8)) {
          continue;
        }

        int offset = fifoPushedX_ - spriteX;
        ...
        uint8_t spriteColorIndex = (high << 1) | low;

        if (spriteColorIndex == 0) {
          continue;  // ignore transparent color
        }

        if (!fetchedSprites_[i].priority || colorIndex == 0) {
          color = fetchedSprites_[i].palette
                      ? sprite2Palette_[spriteColorIndex]
                      : sprite1Palette_[spriteColorIndex];
        }
      }
    }
  ...
  /* spdlog::info("Pushing to sprite FIFO"); */
  /* if (spriteFIFO_.size() <= 8) { */
  /*     for (int i = 0; i < fetchedSprites_.size(); i++) { */
  /*         spdlog::info("fetched sprite: {}", fetchedSprites_[i].x); */
  /*         int spriteX = (fetchedSprites_[i].x - 8) + scx_ % 8; */
  /*         /1* if (backgroundFIFO_.size() > (spriteX + 8)) { *1/ */
  /*         /1*     continue; *1/ */
  /*         /1* } *1/ */

  /*         int offset = backgroundFIFO_.size() - spriteX; */
  /*         /1* if (offset < 0 || offset > 7) { *1/ */
  /*         /1*     continue; *1/ */
  /*         /1* } *1/ */
  ...
  /*         spdlog::info("Pushed to sprite fifo"); */
  /*     } */
  /* } */
}
  ...
  switch (addr) {
    ...
    case 0xFF48:
      ...
      UpdatePaletteSelections(
          value & 0b11111100,
          sprite1Palette_);  // for obj the index 0 is ignored
      return;
    case 0xFF49:
      obp1_ = value;
...
PPU::Mode PPU::GetMode() { return (Mode)(stat_ & 0b11); }
...
bool PPU::GetLCDStat(uint8_t type) { return stat_ & type; }

void PPU::SetLCDStat(uint8_t type) { stat_ |= type; }

void PPU::ClearLCDStat(uint8_t type) { stat_ &= ~type; }

bool PPU::GetLCDControl(uint8_t type) { return lcdc_ & type; }

void PPU::SetLCDControl(uint8_t type) { lcdc_ |= type; }

void PPU::ClearLCDControl(uint8_t type) { lcdc_ &= ~type; }

uint16_t PPU::GetBackgroundTileMapArea() {
  return GetLCDControl(kLCDControlBGTileMapAreaSelect)
             ? kLCDControlBGTileMapArea1
             : kLCDControlBGTileMapArea0;
}

uint16_t PPU::GetTileDataArea() {
  return GetLCDControl(kLCDControlBGWinTileDataAreaSelect)
             ? kLCDControlBGWinTileDataArea1
             : kLCDControlBGWinTileDataArea0;
}

uint16_t PPU::GetWindowTileMapArea() {
  return GetLCDControl(kLCDControlWindowTileMapAreaSelect)
             ? kLCDControlWindowTileMapArea1
             : kLCDControlWindowTileMapArea0;
}
