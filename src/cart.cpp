...
Cart::Cart(std::vector<uint8_t>& cartridge)
        : buffer_(std::move(cartridge)),
          size_(buffer_.size()),
          entry_(buffer_.begin() + kEntryStart, buffer_.begin() + kEntryStart + kEntrySize),
          logo_(buffer_.begin() + kLogoStart, buffer_.begin() + kLogoStart + kLogoSize),
          title_(buffer_.begin() + kTitleStart, buffer_.begin() + kTitleStart + kTitleSize),
          newLicenseeCode_((buffer_[kNewLicenseeStart] << 8) | (buffer_[kNewLicenseeStart + 1])),
          ...
          globalChecksum_((buffer_[kGlobalChecksum] << 8) | (buffer_[kGlobalChecksum + 1])) {
