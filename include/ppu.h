...
const uint8_t kLCDControlWindowTileMapAreaSelect =
    0x01 << 6;  // same as bg tile map area
const uint8_t kLCDControlWindowEnable = 0x01 << 5;
const uint8_t kLCDControlBGWinTileDataAreaSelect =
    0x01 << 4;  // if set, 0x8000, otherwise 0x8800
const uint8_t kLCDControlBGTileMapAreaSelect =
    0x01 << 3;  // if set, 0x9C00, otherwise 0x9800
...
const uint32_t kDefaultColors[4] = {0xFFFFFFFF, 0xFFA9A9A9, 0xFF545454,
                                    0xFF000000};
...
class PPU : public Addressable {
 public:
  ...
  std::vector<uint32_t> screenBuffer_ =
      std::vector<uint32_t>(kLCDHeight * kLCDWidth);
  ...
  uint8_t backgroundPalette_[4];  // holds the palette selections as indices of
                                  // the defaultColors
  ...
  uint8_t sprite1Palette_[4];  // holds the palette selections as indices of the
                               // defaultColors
  uint8_t sprite2Palette_[4];  // holds the palette selections as indices of the
                               // defaultColors
